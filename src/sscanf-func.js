				...
				switch (format.charAt(i + 1)) {
					...
					case 'i':
						...
						j = _addNext(j, pattern, function (num, _, hex, oct) {
							if (hex) return parseInt(num, 16);
							else if (oct) return parseInt(num, 8);
							else return parseInt(num, 10);
							/* 
							 * refactored this ternary since prettier and eslint misbehave
							 * see github.com/prettier/prettier/issues/5814
							  return hex
                  ? parseInt(num, 16)
                  : oct
                    ? parseInt(num, 8)
